...
export default async function MarketingPage() {
  ...
  const [pixelsRes, sheetsRes, waRes, tgRes] = await Promise.all([
    ...
    admin.from("telegram_integrations").select("*").eq("store_id", ctx.store.id).maybeSingle(),
  ]);
  ...
  const tgRow = (tgRes.data ?? null) as Record<string, unknown> | null;
        ...
        {canManage ? (
          <MarketingManager
            ...
            telegram={
              tgRow
                ? {
                    chat_id: (tgRow.chat_id as string) ?? "",
                    enabled_events: (tgRow.enabled_events as string[]) ?? ["new_order"],
                    is_active: (tgRow.is_active as boolean) ?? true,
                    status: (tgRow.status as string) ?? "pending",
                    last_error: (tgRow.last_error as string | null) ?? null,
                    last_test_at: (tgRow.last_test_at as string | null) ?? null,
                    has_token: Boolean(tgRow.bot_token_encrypted),
                  }
                : null
            }
