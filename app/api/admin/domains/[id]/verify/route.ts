...
/**
 * POST /api/admin/domains/[id]/verify — verifies the ownership TXT record.
 * A domain is marked verified only after the expected DNS token is observed.
 */
  ...
  try {
    ...
    if (!check.ok) {
      ...
      await logAudit({
        ...
        entity: "domain",
        entityId: id,
        metadata: { hostname, detail: check.detail },
      });
