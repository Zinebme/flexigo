...
export default function DarPreview(){
 return <div dir="rtl" className="dar-root min-h-screen" style={darCssVars()}>
  ...
  <main>
   ...
   <section className="dar-section bg-[var(--dar-paper)]"><DarContainer><DarHeading eyebrow="HOME INSPIRATION" title="إلهام للبيت" subtitle="أفكار للمطبخ، الترتيب وطاولة البيت"/><div className="grid gap-3 sm:grid-cols-3">{["/images/dar/inspiration.svg","/images/dar/table.svg","/images/dar/organize.svg"].map((img)=><div key={img} className="dar-paper relative aspect-[4/3] overflow-hidden"><StorefrontImage src={img} alt="إلهام للبيت" fill sizes="33vw" className="object-cover"/></div>)}</div><div className="mt-6 grid gap-3 sm:grid-cols-4">{["عملي يومياً","الدفع عند الاستلام","توصيل 58 ولاية","خيارات ديناميكية"].map((x,i)=><div key={x} className="dar-paper p-5"><span className="dar-chip">0{i+1}</span><div className="mt-3 text-sm font-black">{x}</div></div>)}</div></DarContainer></section>
  </main>
 </div>
}