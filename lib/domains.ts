...
export function verificationRecord(hostname: string, token: string): { name: string; value: string; type: "TXT" } {
  return {
    type: "TXT",
    name: `_marqova-verify.${hostname}`,
    value: `marqova-verify=${token}`,
  };
}
...
export async function verifyDomainByDns(hostname: string, token: string): Promise<DnsCheckResult> {
  const names = [`_marqova-verify.${hostname}`, `_flexigo-verify.${hostname}`];
  ...
  return {
    ok: false,
    detail: `Aucun enregistrement trouvé pour ${names[0]}. Ajoutez l'enregistrement TXT chez votre fournisseur DNS, puis relancez la vérification (le DNS peut mettre quelques minutes à se propager).`,
  };
}
