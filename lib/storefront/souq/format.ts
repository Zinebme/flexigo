...
export function formatSouqPrice(cents: number, lang: StoreLanguage | string | null = "ar", currency = "DZD"): string {
  ...
  // Direction is handled by the price element (`dir="ltr"` + `.souq-price`).
  // Keeping control characters out of the value also makes copied prices,
  // search indexes and server-rendered text deterministic.
