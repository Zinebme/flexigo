...
export const productSchema = z.object({
  ...
  description: z.string().trim().max(12000).optional().or(z.literal("")).nullable(),
  short_description: z.string().trim().max(500).optional().or(z.literal("")).nullable(),
  price: priceDA,
  ...
  shipping_label: z.string().trim().max(120).optional().or(z.literal("")).nullable(),
  stock_tracking_mode: z.enum(["none","global","variants"]).default("global"),
  related_product_ids: z.array(uuid).max(20).optional().default([]),
  cross_sell_product_ids: z.array(uuid).max(20).optional().default([]),
  page_element_order: z.array(z.enum(["gallery","title","price","variants","offers","description","order_form","landing","reviews","related"])).max(10).optional().default(["gallery","title","price","variants","offers","description","order_form","landing","reviews","related"]),
  option_groups: z.array(z.object({
    ...
    max_selections: z.number().int().min(1).max(20).default(1),
    values: z.array(z.object({
      value: z.string().trim().min(1).max(60),
      label: z.string().trim().max(80).optional().nullable(),
      color: z.string().max(20).optional().nullable(),
      image: imageUrl.optional().nullable(),
      addon_product_id: uuid.optional().nullable(),
...
export const wizardSchema = z.object({
  ...
  manual_shipping_zones: z.array(z.object({
    wilaya_code: z.number().int().min(1).max(58),
    home_fee: z.number().min(0).max(100000),
    office_fee: z.number().min(0).max(100000),
    is_active: z.boolean().default(true),
  })).max(58).default([]),
