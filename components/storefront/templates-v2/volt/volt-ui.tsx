...
export function VoltPrice({cents,compare,lang,currency="DZD"}:{cents:number;compare?:number|null;lang:StoreLanguage|string|null;currency?:string}){return <div className="flex items-baseline gap-2"><strong className="text-lg text-[var(--volt-blue)]">{formatSouqPrice(cents,lang,currency)}</strong>{compare&&compare>cents?<span className="text-xs text-slate-500 line-through">{formatSouqPrice(compare,lang,currency)}</span>:null}</div>}
