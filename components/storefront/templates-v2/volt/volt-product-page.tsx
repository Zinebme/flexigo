...
export async function VoltProductPage({data,productSlug}:{data:StorefrontData;productSlug:string}){
 ...
 return <>
  <VoltContainer className="py-5 sm:py-8">
   <nav className="mb-5 flex items-center gap-2 overflow-x-auto text-[11px] font-bold text-slate-500"><Link href={data.base}>الرئيسية</Link><span>/</span><Link href={`${data.base}/boutique`}>المتجر</Link><span>/</span><span className="truncate text-slate-300">{product.name}</span></nav>
