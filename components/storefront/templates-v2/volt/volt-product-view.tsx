 ...
 return <div className="grid gap-7 lg:grid-cols-2 lg:gap-10">
  <div>
   <div className="volt-panel relative aspect-square overflow-hidden">
    {current?<StorefrontImage src={current} alt={data.product.name} fill priority sizes="(max-width:1024px) 100vw,50vw" className="object-cover"/>:<div className="flex h-full items-center justify-center text-7xl text-slate-300">⌁</div>}
    <div className="absolute start-3 top-3 volt-badge">VOLT / PRODUCT</div>
   </div>
   {images.length>1?<div className="mt-3 flex gap-2 overflow-x-auto">{images.map((img,i)=><button key={img+i} type="button" onClick={()=>setActive(i)} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border ${active===i?"border-[var(--volt-blue)]":"border-[var(--volt-border)]"}`}><StorefrontImage src={img} alt="" fill sizes="64px" className="object-cover"/></button>)}</div>:null}
  </div>
  <div>
   ...
   <div className="mt-4 flex items-baseline gap-3"><strong className="text-2xl font-black text-[var(--volt-blue)]">{formatSouqPrice(displayPrice,data.lang,data.currency)}</strong>{data.product.compareAtPriceCents&&data.product.compareAtPriceCents>displayPrice?<span className="text-sm text-slate-500 line-through">{formatSouqPrice(data.product.compareAtPriceCents,data.lang,data.currency)}</span>:null}</div>
   {data.description?<p className="mt-4 line-clamp-4 text-sm leading-7 text-slate-600">{data.description}</p>:null}
   <div className="mt-5 grid grid-cols-3 gap-2 text-[10px] font-bold text-slate-600">
    <div className="volt-panel flex min-h-16 flex-col items-center justify-center gap-1 p-2"><VoltIcon name="cash" className="h-4 w-4 text-[var(--volt-blue)]"/>الدفع عند الاستلام</div>
