...
export async function VoltSection({data,section}:{data:StorefrontData;section:Section;index?:number}){
 ...
 if(section.type==="hero"){
  ...
  return <section className="relative overflow-hidden border-b border-[var(--volt-border)] volt-glow">
   <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_15%,rgba(76,111,255,.12),transparent_36%)]"/>
   <VoltContainer className="relative grid min-h-[540px] items-center gap-8 py-10 lg:grid-cols-2 lg:py-14">
    <div className="order-2 lg:order-1">
     ...
     {s.subtitle?<p className="mt-4 max-w-xl text-sm leading-8 text-slate-600 sm:text-base">{String(s.subtitle)}</p>:null}
    ...
    <div className="order-1 lg:order-2"><div className="volt-panel relative aspect-square overflow-hidden">{image?<StorefrontImage src={image} alt={String(s.title||data.name)} fill priority sizes="(max-width:1024px) 100vw,50vw" className="object-cover"/>:<div className="flex h-full items-center justify-center text-8xl text-slate-300">⌁</div>}<div className="absolute inset-x-3 bottom-3 flex items-center justify-between rounded-xl border border-white/10 bg-white/90 px-4 py-3 text-[11px] backdrop-blur"><span className="font-bold text-slate-700">PERFORMANCE / READY</span><span className="text-[var(--volt-blue)]">● ONLINE</span></div></div></div>
   </VoltContainer>
  </section>
 }
 ...
 if(section.type==="products"){
  ...
  return <section className="volt-section bg-[var(--volt-surface-2)]"><VoltContainer><VoltHeading eyebrow="GEAR" title={String(s.title||"المنتجات")} subtitle={s.subtitle?String(s.subtitle):null} action={<a href={`${data.base}/boutique`} className="text-xs font-black text-[var(--volt-blue)]">عرض الكل ←</a>}/><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 sm:gap-5">{list.map(p=><VoltProductCard key={p.id} product={p} base={data.base} lang={data.lang} currency={data.currency}/>)}</div></VoltContainer></section>
 }
 if(section.type==="features"){
  const items=(s.items as Array<{title?:string|null;text?:string|null}>|undefined)??[];
  return <section className="volt-section"><VoltContainer><VoltHeading eyebrow="WHY VOLT" title={String(s.title||"المزايا")} subtitle={s.subtitle?String(s.subtitle):null}/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{items.map((it,i)=><div key={i} className="volt-panel p-5"><div className="text-2xl font-black text-[var(--volt-blue)]">0{i+1}</div><h3 className="mt-4 text-sm font-black">{it.title}</h3><p className="mt-2 text-xs leading-6 text-slate-500">{it.text}</p></div>)}</div></VoltContainer></section>
 }
 if(section.type==="banner"){
  const image=(s.desktop_image||s.mobile_image) as string|undefined;
  return <section className="volt-section"><VoltContainer><div className="volt-panel relative overflow-hidden"><div className="grid items-center lg:grid-cols-2"><div className="p-7 sm:p-10"><div className="volt-kicker">UPGRADE MODE</div><h2 className="volt-title mt-3 text-3xl">{String(s.title||"ترقية ذكية")}</h2>{s.subtitle?<p className="mt-3 text-sm leading-7 text-slate-600">{String(s.subtitle)}</p>:null}<a href={String(s.button_link||"/boutique")} className="volt-btn mt-6">{String(s.button_text||"اكتشف")}</a></div><div className="relative aspect-[16/10] lg:aspect-square">{image?<StorefrontImage src={image} alt={String(s.title||"")} fill sizes="(max-width:1024px) 100vw,50vw" className="object-cover"/>:null}</div></div></div></VoltContainer></section>
 }
 if(section.type==="reviews"){
  const reviews=await loadSouqReviews(data.id,null,6); if(!reviews.length) return null;
  return <section className="volt-section bg-[var(--volt-surface-2)]"><VoltContainer><VoltHeading eyebrow="REVIEWS" title={String(s.title||"آراء الزبائن")} subtitle={s.subtitle?String(s.subtitle):null}/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{reviews.map(r=><figure key={r.id} className="volt-panel p-5"><div className="text-[var(--volt-amber)]">{"★".repeat(r.rating)}</div>{r.title?<div className="mt-3 text-sm font-black">{r.title}</div>:null}<blockquote className="mt-2 text-xs leading-6 text-slate-600">{r.body}</blockquote><figcaption className="mt-4 text-[11px] font-bold text-slate-500">{r.customerName}</figcaption></figure>)}</div></VoltContainer></section>
 }
 if(section.type==="faq"){
  const faq=await loadSouqFaq(data.id,Number(s.max_items||8)); if(!faq.length) return null;
  return <section className="volt-section"><VoltContainer className="max-w-3xl"><VoltHeading eyebrow="FAQ" title={String(s.title||"الأسئلة الشائعة")} subtitle={s.subtitle?String(s.subtitle):null}/><div>{faq.map(f=><details key={f.id} className="border-b border-[var(--volt-border)] py-4"><summary className="cursor-pointer text-sm font-extrabold">{f.question}</summary><p className="pt-3 text-sm leading-7 text-slate-600">{f.answer}</p></details>)}</div></VoltContainer></section>
 }
